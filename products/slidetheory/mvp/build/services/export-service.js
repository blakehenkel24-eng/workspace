 ...
 * Handles PowerPoint and PDF export generation
 */
  ...
  if (content.keyPoints && content.keyPoints.length > 0) {
    content.keyPoints.forEach((point, index) => {
      ...
      slide.addText(point.heading, {
        ...
        fontSize: 13, fontFace: 'Arial', bold: true, color: COLORS.black
      });
      
      // Point text
      slide.addText(point.text, {
        x: 1.0, y: yPos + 0.25, w: 8.5, h: 0.5,
        fontSize: 11, fontFace: 'Arial', color: COLORS.grayDark, valign: 'top'
      });
...
function addMarketAnalysisContent(slide, content) {
  ...
  if (content.marketSize) {
    ...
    slide.addText('Total Addressable Market', {
      ...
      fontSize: 10, fontFace: 'Arial', color: COLORS.grayDark
    });
    
    slide.addText(content.marketSize, {
      x: 0.7, y: yPos + 0.3, w: 3.6, h: 0.4,
      fontSize: 18, fontFace: 'Arial', bold: true, color: COLORS.navy
    });
  ...
  if (content.insights && content.insights.length > 0) {
    content.insights.forEach((insight, index) => {
      slide.addText('→ ' + insight, {
        ...
        fontSize: 11, fontFace: 'Arial', color: COLORS.black, valign: 'middle'
      });
    });
  }
  ...
  if (content.chartData) {
    slide.addShape('rect', {
      ...
      fill: { color: COLORS.gray }, rectRadius: 0.1
    });
    ...
    content.chartData.labels.forEach((label, index) => {
      ...
      slide.addText(label, {
        ...
        fontSize: 9, fontFace: 'Arial', color: COLORS.grayDark, align: 'center'
      });
      
      slide.addText(String(value) + '%', {
        x: xPos, y: yBarPos - 0.25, w: barWidth, h: 0.2,
        fontSize: 10, fontFace: 'Arial', bold: true, color: COLORS.black, align: 'center'
      });
    });
  }
}
  ...
  if (content.metrics && content.metrics.length > 0) {
    ...
    content.metrics.forEach((metric, index) => {
      ...
      slide.addText(metric.label.toUpperCase(), {
        ...
        fontSize: 9, fontFace: 'Arial', color: COLORS.grayDark
      });
      
      slide.addText(metric.value, {
        x: xPos + 0.15, y: 1.55, w: cardWidth - 0.3, h: 0.35,
        fontSize: 20, fontFace: 'Arial', bold: true, color: COLORS.navy
      });
      ...
      slide.addText(metric.change + ' ' + (metric.period || ''), {
        ...
        fontSize: 11, fontFace: 'Arial', bold: true, color: changeColor
      });
    });
  }
    ...
    if (content.tableData.rows) {
      content.tableData.rows.forEach((row, rowIndex) => {
        ...
        row.forEach((cell, colIndex) => {
          ...
          slide.addText(cell, {
            x: 0.5 + (colIndex * colWidth), y: yPos, w: colWidth, h: rowHeight,
            fontSize: 10, fontFace: 'Arial', color: COLORS.black,
            align: colIndex === 0 ? 'left' : 'right', valign: 'middle'
          });
        });
      });
    }
...
function addFooter(slide, footer) {
  ...
  if (footer && footer.source) {
    slide.addText(`Source: ${footer.source}`, {
      ...
      fontSize: 8, fontFace: 'Arial', color: COLORS.grayDark
    });
  }
  ...
  slide.addText('Confidential', {
    ...
    color: COLORS.grayDark, align: 'right'
  });
}

/**
 * Generate PDF from slide content
 */
async function generatePDF({ slideType, content, outputPath }) {
  ...
  try {
    ...
    // Build HTML for the slide using templates
