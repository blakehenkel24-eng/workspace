/**
 * Slide Templates
 * HTML templates and styles for each slide type
 */

const config = require('../config');
const { SLIDE_WIDTH, SLIDE_HEIGHT } = config.slides.dimensions;

/**
 * Get CSS styles for slides
 */
...
function buildGrowthStrategyHTML(content) {
  const flywheelItems = content.flywheel || [
    { label: 'Attract' }, { label: 'Engage' }, { label: 'Delight' }, { label: 'Retain' }
  ];
...
/**
 * Build slide body based on type
 */
function buildSlideBody(slideType, content) {
  switch (slideType) {
